...
const Pagination = props => {
    ...
    return (
        <ArrowBar>
            <NavLink to={`/${props.page > 1 ? props.page - 1 : 1}`}><PageButton onClick = {() => changePage(false)}><ArrowImg src={backArrow} /></PageButton></NavLink>
            <h3>Page {props.page}</h3>
            <NavLink to={`/${props.page + 1}`}><PageButton onClick = {() => changePage(true)}><ArrowImg src={forwardArrow} /></PageButton></NavLink>
        </ArrowBar>
    );
};
